...
const PostPrivacyModal = ({visible, onClose ,userinfo, data , handleSelectPrivacy, darkMode}) => {

  const isMonetizationDisabled = userinfo?.have_monetization === '0' 
    ...
    <TouchableOpacity style={styles.modalContainer} onPress={onClose}>
      <TouchableOpacity activeOpacity={1} style={[styles.modalContent,darkMode == 'enable' ? {backgroundColor: COLOR.DarkThemLight} : {backgroundColor: COLOR.White}]}>
        <View style={styles.modalHeader}>
          <Text style={[styles.modalHeaderText,darkMode == 'enable' ? {color : COLOR.White} : {color: COLOR.Grey500,}]}>{i18n.t(`translation:postprivacy`)}</Text>
        ...
        <View style={{padding: 16, rowGap: 16}}>
          ...
          {data.map((item, index) => (
              <TouchableOpacity
                ...
                disabled={isMonetizationDisabled && item.id === 6 }// Disable based on monetization
                style={
                  isMonetizationDisabled && item.id === 6 && { opacity: 0.5 } // Apply reduced opacity when disabled
                }
              >
...
export default PostPrivacyModal;
