...
const Post = props => {
    ...
    async (postId = 0) => {
      ...
      try {
        ...
        if (data.api_status === 200) {
          ...
          // setLimit(page !== 1 && 5)
          dispatch(addNewPostList(mergedData));
        }
      } catch (error) {
        console.error(error);
      } finally {
  ...
  return (
    // <Pressable onPressIn={() => setReactEnable(!isReactEnable)}>
    <View style={styles.container}>
      <FlatList
        ...
        maxToRenderPerBatch={page === 1 ? 10 : 3}
        windowSize={3}
  ...
  ({
    ...
    return (
      <View
        ...
        {item.reaction ? <ReactCommentShareStatus item={item} /> : null}
