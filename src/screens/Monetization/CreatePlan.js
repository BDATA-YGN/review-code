...
import { updatePlan,setMonetizationList } from '../../stores/slices/monetization_slice';
import { useDispatch,useSelector } from 'react-redux';
import { getCurrecyLists, requestUnSubscribed } from '../../helper/Monetization/MonetizationModel';
const CreatePlan = ({route}) => {
  const navigation = useNavigation();
  const [currenyModalVisible, setcurrenyModalVisible] = useState(false);
  const [durationModalVisible,setDurationModalVisible]= useState(false);
  const {darkMode,currencyLists} = route.params;
  ...
  const monetizationList = useSelector(state => state.MonetizedSlice.plans);
  ...
  const onPressCreatePlan = () => {
    if (name && price && currencyId !== ''  && period && description) {
      console.log("Currency ID:", currencyId); // Debugging: check currencyId before submitting
      setIsLoading(true);
      submitNewPlan('add', name, price, currencyId, period, description)
        .then(value => {
          setIsLoading(false);
          if (value.api_status === 200) {
            ...
            dispatch(setMonetizationList([
              ...monetizationList,
              { name, price, currencyId, period, description },
            ]));
            ...
            Alert.alert('Error', 'Monetization cannot be Created 4 plans', [
              { text: 'OK' },
            ], { cancelable: false });
          }
        })
        .catch(error => {
          ...
          console.error('Error creating plan:', error); // Log error for debugging
        });
    } else {
    ...
    <SafeAreaView style={darkMode == 'enable' ? styles.DSafeAreaView :styles.SafeAreaView}>
      ...
      <View style={{padding: SPACING.sp20}}>
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <TouchableOpacity
            style={[styles.timeCard, {backgroundColor: darkMode == 'enable' ? COLOR.DarkThemLight : COLOR.White100,flex: 0.4}]}
            activeOpacity={0.8}
