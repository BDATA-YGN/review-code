  ...
  const onSubmitFollow = async (user_id, index) => {
    try {
      ...
      if (data.api_status === 200) {
        ...
        newData[index].is_friend_confirm =
          data.follow_status === 'requested' ? 1 : 0;
  ...
  const renderUserItem = ({item, index}) => (
    <View style={styles.profileHolder} key={index}>
      <TouchableOpacity
        ...
        {
            userInfoData.user_id === item.user_id ? null : 
            (
                <TouchableOpacity
...
export default OtherUserSeeAllFriends
