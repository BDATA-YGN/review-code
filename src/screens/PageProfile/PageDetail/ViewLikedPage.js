...
const ViewLikedPage = ({route}) => {
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      console.log('balance', userDataResponse.user_data.avatar);
    } catch (error) {
      fetchUserInfo();
    }
  };
    ...
    likeUnlineAction(page_id).then(async data => {
      if (data.api_status == 200) {
        if (data.like_status == 'liked') {
          setLiked(en.unLike);
          watchIdEnable === '1' && setWatchId('liked');
        } else {
          setLiked(en.like);
  ...
  const renderParentItem = ({item}) => (
    <View style={{marginBottom: 20}}>
      ...
      <View
        style={
          darkMode === 'enable' ? styles.DuserInfoHolder : styles.userInfoHolder
        }>
            ...
            <TouchableOpacity
              ...
              <View
                ...
                <IconPic
                  width={20}
                  height={20}
                  source={IconManager.message_light}
                />
                <SizedBox width={10} />
                <Text
                  style={
          ...
          <View style={{width: '90%', justifyContent: 'flex-start'}}>
            {pageInfo?.likes !== '' && (
              <View
                style={
                  darkMode === 'enable'
                    ? styles.DuserInfoProfileContent
                    : styles.userInfoProfileContent
                }>
                <IconPic
                  source={
                    darkMode === 'enable'
      ...
      {/* <SizedBox height={SPACING.sp12} width={'100%'} color={COLOR.White50} /> */}

      {/* <View>
        <CreatePost postType={stringKey.navigateToMyPage} userId={myNavigatedId} userData={pageInfo} />
      </View> */}
