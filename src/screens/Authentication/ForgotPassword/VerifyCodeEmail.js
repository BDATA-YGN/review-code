...
const VerifyCodeEmail = props => {
  ...
  const handleForgotPasswordCode = async () => {
    ...
    try {
      const data = await submitForgotPasswordCode({
        email: props.route.params.email_address,
        code: otpCode,
      });
      ...
      if (data.api_status === 200) {
        ...
        // setTimeout(() => {
        //   navigation.navigate('ResetPassword',{ email: props.route.params.email,code : otpCode})
        // }, 1000);
      } else {
  ...
  const handleResendEmail = async () => {
    const email = props.route.params.email_address; // Extract the email from mandatoryValues
    setOtp(['', '', '', '', '', '']); // Reset the OTP array
    setOtpCode(''); // Clear the OTP code
    inputRefs[0].current.focus(); // Set focus back to the first input field
    setIsButtonDisabled(true); // Disable the button
      ...
      <View style={{padding: 16}}>
        ...
        <View>
          <Text
            ...
            <Text style={[
              ...
              {' '}
              {props.route.params?.email_address}
            </Text>
          </Text>
        </View>
        ...
        <View style={{marginTop: 32, gap: 8}}>
          ...
          <View style={{flexDirection: 'row', justifyContent: 'center'}}>
            {canResend ? (
              <TouchableOpacity onPress={handleResendEmail} style = {{ flexDirection : 'row'}}>
