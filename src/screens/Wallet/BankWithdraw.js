...
const BankWithdraw = ({route}) => {
  ...
  const onPressCreateWithdrawlBank = () => {
    ...
    if (amount && name && bank) {
      console.log('Selected User ID:', name); // Log user_id for debugging
      console.log('Amount:', amount); // Log amount for debugging
      setIsLoading(true);
      submitBankWithdraw('bank', amount,bank, name )
        .then(value => {
  ...
  return (
    <SafeAreaView style={darkMode == 'enable' ? styles.DsafeAreaView : styles.safeAreaView}>
      <ActionAppBar
        appBarText="Withdrawal With Bank"
      ...
      <View style={darkMode == 'enable' ? styles.Dcard :styles.card}>
        ...
        <View style={darkMode == 'enable' ? styles.DtimeCard :styles.timeCard}>
            <TextInput
              ...
              placeholderTextColor={darkMode == 'enable' ? COLOR.White100 :COLOR.Grey300}
              value={bank}
              onChangeText={text => setBank(text)} // Add this line to update the amount state
            />
          </View>
          <SizedBox height={SPACING.sp20} />
        <View style={darkMode == 'enable' ? styles.DtimeCard :styles.timeCard}>
            <TextInput
              ...
              placeholderTextColor={darkMode == 'enable' ? COLOR.White100 :COLOR.Grey300}
              value={name}
              onChangeText={text => setName(text)} // Add this line to update the amount state
            />
          </View>
          <SizedBox height={SPACING.sp20} />
        ...
        <ActionButton
          text='Request'
          onPress={() => onPressCreateWithdrawlBank(amount, name)}
        />
